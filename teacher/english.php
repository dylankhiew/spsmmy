    ...
    <script type="text/javascript">
      ...
      function drawChart() {
        var data = google.visualization.arrayToDataTable([
          ...
          <?php
          $sql = "SELECT * FROM marks,teacher WHERE marks.class_id = '$cid' ORDER BY marks.english";
          $result = mysqli_query($conn,$sql);
          while ($row = mysqli_fetch_assoc($result)) {
            echo "[".$row['english'].",".$row['english'].", 50],";
        ...
        var options = {
          ...
          hAxis: {minValue: 0,maxValue:100, title: 'English',  titleTextStyle: {color: '#333'}},
          vAxis: {minValue: 0},
          width:950,
          height:500,
    ...
    <div class="container">
      <div class="jumbotron" style="opacity: 0.95;border-radius:25px">
        <span style="text-align: left" class="login100-form-title p-b-48">
          <a><font size="6">English</font></a><br>
        </span>
        <!-- Search -->
                ...
                <div class="limiter">
                      <form class="login100-form validate-form" method="post">
                        <table class="table table-bordered">
                         ...
                         <?php
                         $i=0;
                         $result=mysqli_query($conn,"SELECT * from marks,teacher where  marks.class_id = teacher.teacher_id ORDER BY student_name");
                           ...
                           while($row = mysqli_fetch_assoc($result))
                           {
                             ...
                             if($row["english"]>=90)
                             {
                               ...
                               <tr class="alttr1">
                                 ...
                                 <td><font size="4"><input class="input100" type="text" name="mark[]" class="form-control"  value="<?php echo $row["english"]; ?>" placeholder="No marks"></font></td>
                                 <td><font size="4"><input class="input100" type="text"  class="form-control"  value="A+" readonly></font></td>
                                </tr>
                               <?php
                             }else if($row["english"]>=80){
                               ?>
                               <tr class="alttr1">
                                 <td><a><font size="4"><?php echo $i;?></font></a></td>
                                 <td><a><font size="4" color="black"><?php echo $row["student_name"]; ?><input type="hidden" name="sid[]" value="<?php echo $row["student_id"]; ?>"></font></a></td>
...
if(isset($_POST["update"]))
{
  ...
  mysqli_query($conn,"UPDATE marks SET english = '$mark' WHERE student_id = '$sid'");

    }
	?>
      <script type = "text/javascript">
				alert("Successful updated marks for English.");
			</script>
	<?php
